        ...
        protected override Task<bool> UpdateDevice(Dictionary<DeviceKeys, SimpleColor> keyColors, DoWorkEventArgs e, bool forced = false)
        {
            ...
            try
            {
                foreach (KeyValuePair<DeviceKeys, SimpleColor> key in keyColors)
                {
                    ...
                    if (Global.DeviceConfig.VarRegistry.GetVariable<bool>($"{DeviceName}_enable_generic"))
                    {
                    ...
                    if (DeviceKeysMap.TryGetValue(devKey, out byte i))
                    {
                ...
                if (Global.DeviceConfig.VarRegistry.GetVariable<bool>($"{DeviceName}_enable_ryos"))
                {
