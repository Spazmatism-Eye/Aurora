...
namespace AuroraRgb.Profiles.QuantumConumdrum
{
    ...
    /// </summary>
    public partial class Control_QuantumConumdrum : UserControl
    {
        ...
        private bool InstallWrapper(string installpath = "")
        {
            ...
            if (!String.IsNullOrWhiteSpace(installpath))
            {
                ...
                using (BinaryWriter lightfx_wrapper_32 = new BinaryWriter(new FileStream(path, FileMode.Create)))
                {
                    lightfx_wrapper_32.Write(Properties.Resources.Aurora_LightFXWrapper86);
                }
        ...
        {
            String installpath = SteamUtils.GetGamePath(200010);
            if (!String.IsNullOrWhiteSpace(installpath))
            {
                string path = System.IO.Path.Combine(installpath, "Binaries", "Win32", "LightFX.dll");
