...
public partial class GameStateParameterPicker : INotifyPropertyChanged {
    ...
    public VariablePath WorkingPath { get; set; } = VariablePath.Empty;
    ...
    public IEnumerable<GameStateParameterLookupEntry> CurrentParameterListItems {
        get {
            ...
            if (Application.ParameterLookup.IsValidParameter(WorkingPath.GsiPath))
                GoUp();
    ...
    /// </summary>
    public VariablePath SelectedPath
    {
        get => (VariablePath)GetValue(SelectedPathProperty);
        set => SetValue(SelectedPathProperty, value);
    }

    public static readonly DependencyProperty SelectedPathProperty =
        DependencyProperty.Register(nameof(SelectedPath), typeof(VariablePath), typeof(GameStateParameterPicker),
            new FrameworkPropertyMetadata(VariablePath.Empty, FrameworkPropertyMetadataOptions.BindsTwoWayByDefault, SelectedPathDPChanged));
    ...
    private static void SelectedPathDPChanged(DependencyObject sender, DependencyPropertyChangedEventArgs e) {
        ...
        if (!picker.ValidatePath((VariablePath)e.NewValue)) {
            picker.SelectedPath = VariablePath.Empty;
    ...
    /// </summary>
    private bool ValidatePath(VariablePath path) =>
        ...
        while (el != null && el != mainListBox) {
            if (el is ListBoxItem item && item.DataContext is GameStateParameterLookupEntry itemContext) {
                ...
                if (itemContext.IsFolder) {
                    ...
                    SelectedPath = new VariablePath(itemContext.Path);
                }
            }
